...
@NotThreadSafe
public class CashFlowSubReportParser implements ISubReportParser {
    ...
    protected void parseRow(final FieldSet fieldSet, final CompanyOperatingReports reports) {
        final CashFlowSubReportParserRow row = CashFlowSubReportParserRow.valueOfTitle(curCategory1, curCategory2,
                curTitle1, curTitle2);
        if (row == null) {
            return;
        }
        final CashFlow report = reports.getCashFlow();
