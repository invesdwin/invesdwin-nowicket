function allowCopyPasteOnDisabledInputs() {
	...
	function replaceWithReadonlyInput(tag, value) {
		...
		if (fixedTag.length) {
			...
			fixedTag.attr("id", tag.attr("id")+"_allowCopyPasteOnDisabledInputs");
			...
			input.attr("id", tag.attr("id")+"_allowCopyPasteOnDisabledInputs");
	...
	$('input:disabled:not([type=radio]):not([type=checkbox]):not([type=file]):visible').after(function(e) {
		var tag = $(this);
		var value = tag.val();
		var input = replaceWithReadonlyInput(tag, value);
		return input;
