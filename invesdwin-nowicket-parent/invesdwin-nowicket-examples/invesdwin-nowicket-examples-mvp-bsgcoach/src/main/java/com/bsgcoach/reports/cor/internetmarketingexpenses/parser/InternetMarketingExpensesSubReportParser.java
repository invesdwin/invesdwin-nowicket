...
@NotThreadSafe
public class InternetMarketingExpensesSubReportParser implements ISubReportParser {
    ...
    protected void parseRow(final FieldSet fieldSet, final CompanyOperatingReports reports) {
        final InternetMarketingExpensesSubReportParserRow row = InternetMarketingExpensesSubReportParserRow.valueOfTitle(
                curCategory1, curCategory2, curTitle1, curTitle2);
        if (row == null) {
            return;
        }
        final InternetMarketingExpenses report = reports.getInternetMarketingExpenses();
