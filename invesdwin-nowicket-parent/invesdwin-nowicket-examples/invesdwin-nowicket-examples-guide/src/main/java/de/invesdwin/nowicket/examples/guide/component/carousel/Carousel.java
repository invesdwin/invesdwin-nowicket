package de.invesdwin.nowicket.examples.guide.component.carousel;
...
/**
 * TODO: remove this class as soon as pull request is merged: https://github.com/l0rdn1kk0n/wicket-bootstrap/pull/592
 * 
 * @author subes
 *
 */
...
public class Carousel extends Panel {
    ...
    private org.apache.wicket.util.time.Duration interval = org.apache.wicket.util.time.Duration.seconds(5);
     ...
     */
    private Component newImageList(final String wicketId) {
        final ListView<ICarouselImage> listView = new ListView<ICarouselImage>(wicketId, model) {

            private boolean renderedActive = false;

            @Override
            protected void populateItem(final ListItem<ICarouselImage> item) {
     ...
     *
     * @param markupId
     *            the markup id of the header
     * @param image
     *            the current image for this header
    ...
    protected Component newDescription(final String markupId, final ICarouselImage image) {
        final Label description = new Label(markupId);
     ...
     */
    public final org.apache.wicket.util.time.Duration getInterval() {
        return interval;
    }

    /**
     * The amount of time to delay between automatically cycling an item. If Duration.NONE or value is 0, carousel will
     ...
     */
    public final Carousel setInterval(final org.apache.wicket.util.time.Duration interval) {
