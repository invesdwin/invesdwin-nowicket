...
public class Palette<T> extends GenericPanel<Collection<? extends T>> {
    ...
    /**
     * One-time init method for components that are created via overridable factories. This method is here because we do
     * not want to call overridable methods form palette's constructor.
     */
    ...
    /**
     * Override this method if you do <strong>not</strong> want to localize the display values of the generated options.
    ...
    /**
     * Returns recorder component. Recorder component is a form component used to track the selection of the palette. It
     * receives <code>onchange</code> javascript event whenever a change in selection occurs.
     * 
    ...
    /**
     * The model object is assumed to be a Collection, and it is modified in-place. Then {@link Model#setObject(Object)}
     * is called with the same instance: it allows the Model to be notified of changes even when
     * {@link Model#getObject()} returns a different {@link Collection} at every invocation.
     * 
     */
    protected final void updateModel() {
        // get the selected choices first, since the available choices might depend on the
        // previously selected objects.
        final Iterator<T> it = getRecorderComponent().getSelectedChoices();

        modelChanging();

        final Collection<T> collection = getModelCollection();
        collection.clear();
        while (it.hasNext()) {
        ...
        @SuppressWarnings("unchecked")
        final IModel<Object> defaultModel = (IModel<Object>) getDefaultModel();
